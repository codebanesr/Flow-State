...
# Bring up the Docker stack in development mode (Certbot is not run).
up:
	docker compose up -d

# Bring up the Docker stack with the production profile enabled (Certbot runs).
prod:
...
help:
	...
	@echo "  make prod    - Bring up the Docker stack in production mode (runs certbot)"
